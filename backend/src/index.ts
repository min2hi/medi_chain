...
// ─── Trust Proxy: BẮT BUỘC trên Render/Heroku/Railway ───────────────────────
// Render đặt server sau reverse proxy và inject 'X-Forwarded-For' header.
// Nếu không set trust proxy = 1, express-rate-limit sẽ ném ValidationError
// (ERR_ERL_UNEXPECTED_X_FORWARDED_FOR) cho MỌI request → toàn bộ /api/* bị 500.
...
app.use('/api/admin/stats',         adminStatsRoutes);          // Admin KPI Dashboard
...
app.use('/api/admin/appointments',   adminAppointmentsRoutes);   // Clinic Appointments
